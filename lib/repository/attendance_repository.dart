...
class AttendanceRepository {
  ...
  Future<Multimap<String, String>> getAttendanceForStudent() async {
    ...
    for (var data in responseData['result']){
      ...
      for (var attendance in data['attendances']){
        ...
        var course = Course(null, courseType, null, null, courseDate, courseNumber);
    ...
    print("full size: " + _attendanceList.length.toString());
  ...
  Future<bool> saveAttendance(String attendanceString) async {
    try {
      ...
      SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
      var studentId = sharedPreferences.get(Constants.studentId);
      var course = Course(courseName, courseType, courseTeacher, courseTeacherId, courseCreatedAt, courseNumber);
      var attendance = Attendance(DateTime.now().toIso8601String(), studentId, attendanceString, course);
      ...
      var response = await http.post(
          ...
          body: {
            'attendance': json.encode(attendance.toJSON())
          });
